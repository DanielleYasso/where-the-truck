...
#######################################
# FLASK-LOGIN USER LOADER FOR SESSION #
#######################################
...
def user_settings():
    ...
    if form.validate_on_submit():
        ...
        # prevents someone from changing a logged in users password
...
def getLastGood(attraction):
    ...
    if attraction.last_good_checkin_id:
        last_good = model.db.session.query(model.Checkin)\
            .get(attraction.last_good_checkin_id)
    else:
        return False
...
def getTimeout(this):
    """ Gets the timeout for a given checkin object, by comparing that 
    checkin's timeestamp with datetime.now() """
...
def checkin():
    ...
    if attraction.checkin_id:
        previous_checkin = model.db.session.query(model.Checkin)\
            .get(attraction.checkin_id)
        ...
        if previous_checkin.user_id and previous_checkin.user.is_trusted():
            ...
            if previous_checkin.calculated_rating != None and \
                previous_checkin.calculated_rating > 0.1:
